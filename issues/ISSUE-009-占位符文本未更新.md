...
- `app/layout.tsx` 的 `Create Next App` / `Generated by create next app` → 改为 `苏畅 | AI 应用开发者` + 作品集描述（取自 `data/personal.ts`）。
- `components/ui/Navigation.tsx` Logo 的 `D` / `Portfolio` → 改为 `personal.name.charAt(0)` / `personal.name`（单一数据源，非硬编码）。
