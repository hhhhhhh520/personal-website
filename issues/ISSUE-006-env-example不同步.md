...
- `.env.example` 已更新为 `ASTRON_API_KEY=your_api_key_here`
- **根因补充**：此前 `.env.example` 从未进版本库——`.gitignore` 的 `.env*` 规则把它一起忽略了，所以「修了也不被跟踪、克隆方仍看不到」。已加 `!.env.example` 例外并纳入提交（模板无真实密钥）。
