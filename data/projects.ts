...
export const projects: Project[] = [
  {
    ...
    fullDesc: '基于 LangGraph 和 LiteLLM 构建的轻量级 AI 编程助手。支持多模型切换（DeepSeek、通义千问、讯飞星辰等），实现工具调用、文件操作、Shell 命令执行等核心功能。采用多 Agent 并发架构，支持任务分解和并行执行。完整测试覆盖，包含 1677 个测试用例。',
    fullDescEn: 'Lightweight AI coding assistant built with LangGraph and LiteLLM. Supports multi-model switching (DeepSeek, Qwen, iFlytek), tool calling, file operations, and shell command execution. Multi-agent concurrent architecture with task decomposition and parallel execution. Complete test coverage with 1677 test cases.',
    techStack: ['Python', 'LiteLLM', 'LangGraph', 'asyncio'],
    highlights: ['1677 测试用例', '多模型支持', '并发 Agent', '工具调用'],
    highlightsEn: ['1677 Test Cases', 'Multi-Model Support', 'Concurrent Agents', 'Tool Calling'],
