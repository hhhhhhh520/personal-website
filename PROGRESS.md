...
| 阶段 | 内容 | 文件 | 完成日期 |
|------|------|------|----------|
| 调整 | `data/projects.ts` 将 AgentHub 提至数组首位（旗舰优先），QuickResume 简历卡 top-4 现为 AgentHub 领衔；/projects 列表页顺序同步变化 | data/projects.ts | 2026-09-04 |

#### P29 — ISSUE-004 决策：保留现状（2026-09-04）
| 阶段 | 内容 | 文件 | 完成日期 |
|------|------|------|----------|
| 决策 | 全客户端渲染保留现状(评估:成本高收益低;真实痛点已由 ISSUE-003 懒加载 + ISSUE-007 LightRays 优化覆盖),不再改造 | issues/ISSUE-004, issues/README | 2026-09-04 |
...
| 阶段 | 内容 | 文件 | 完成日期 |
|------|------|------|----------|
| 优化 | props→ref+挂载一次/60fps帧率上限/prefers-reduced-motion静态帧/空catch补日志（着色器未动） | components/effects/LightRays.tsx | 2026-09-04 |
